...
const int INF = 1e9;
const int iterations = 100; // Можно увеличить до 100 для точности
...
int main() {
    ...
    ofstream sizeFile("src/data/sizes.txt");
    ofstream fwFile("src/data/floyd.txt");
    ofstream bfFile("src/data/bellman.txt");
    ofstream djkFile("src/data/dijkstra.txt");
    if (!sizeFile.is_open() || !fwFile.is_open() || !bfFile.is_open() || !djkFile.is_open()) {
        cerr << "Ошибка открытия файлов вывода!" << endl;
        return 1;
    }
    ...
    cout << left << setw(10) << "Size"
            ...
            // Записываем заголовки в файлы (один столбец в каждом)
    ...
    for (int size : sizes) {
        ...
        cout << left << setw(10) << size
                    ...
                    << endl;
                // Записываем данные в отдельные файлы
                sizeFile << size << "\n";
                fwFile << (timeFW / iterations) << "\n";
                bfFile << (timeBF / iterations) << "\n";
                djkFile << (timeDJK / iterations) << "\n";
    }
